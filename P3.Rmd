...
\newpage
```{r Figure 1, echo=FALSE, fig.cap="Top 10 Mean Difference in Function Words Between LLM and Human Texts", warning=FALSE, fig.width = 6.5, fig.height = 3.5}
...
```{r Figure 2, echo=FALSE, fig.cap="MDS Visualisation of Human and LLM Texts", warning=FALSE, fig.width = 6, fig.height = 4}
...
```{r Figure 3, echo=FALSE, fig.cap="MDS Visualisation of Authors", warning=FALSE, fig.width = 5, fig.height = 3.5}
plot(pts, 
     type="n", 
...
Table 2 below shows the LOOCV results of 'KNN' with k=2, 644 of human texts are classified to LLM, which means that the model is bad on recognizing Human texts. 
...
The kappa value is pretty low, so the true improvement of this model over chance is only moderate. And the sensitivity (Human recall) is much more lower than specificity (LLM recall), means that the model is biased on LLM.
...
```{r, results='asis'}
breaks <- hist(log_diff, plot=FALSE)$breaks

hist(log_diff,
     breaks=breaks,
     col=ifelse(breaks[-length(breaks)] < 0,
                "green",
                "red"),
...
In the Bonus section, we shall start from a binary classifier trained on Human-written texts verus ChatGPT-generated texts to investigate the classification tendency of Genmini texts under the model. The result was shown on the histogram above. The Gemini texts fall on the positive side of the decision boundary, the vertical line $\log\frac{P(x\mid Human)}{P(x mid GPT)} = 0$, is almost equal to the texts fall on the negative side, indicating that under the "Human vs GPT" classifier, there is no strong evidence for the model to distinguish Gemini from either Human or GPT.
...
In addition to the proposal from the Bonus, we further constructed cross-model to classify the tendencies of tested LLM by training on Human-authored texts versus Gemini-authored texts and Human-authored texts versus Llama-authored texts.
...
ggplot(summary_table,
  ...
  theme_minimal()
```
The assessed probability of classifying the tested LLM generated texts as human-authored varies signifiacantly across different models. As shown in the our cross-model summary table, the probability of Gemini and Llama be regarded as Human written under trained GPT and Human model are 47.4% and 61.3% respectively, indicating almost half of the texts generated by Gemini and Llama are wrongly classified as human written, which shows a weak tendency of classifying texts. However, the models trained by Human and Gemini, as well as model trained by Human and Llama, have obvious tendencies on the other LLMs classification. In particular, the proportion of corrected classified as LLM generated exceeded 90% for both GPT and Llama.


The test results also demonstrate whether the category of LLM being tested significantly influences classification outcomes. As shown in the charts above, GPT exhibits the lowest probability of being classified as Human, both below 10%. Although Llama and Gemini's results appear slightly lower compared to GPT's performance under the same model, this suggests these two LLMs may be more challenging to distinguish from human writing. Concurrently, the low predictive efficiency of GPT and HUMAN models towards Llama and Gemini explains how, under this framework, predictive bias may be influenced concurrently by both the model itself and the type of AI being tested.


# Compare GPT-HUMAN binary model with LLM-HUMAN model
