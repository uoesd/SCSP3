...
```{r Table 2, echo=FALSE, fig.cap="AI Accuracy under cross-model summary table", warning=FALSE, fig.width = 6, fig.height = 2.5}
summary_table
```
The assessed probability of classifying the tested LLM generated texts as human-authored varies significantly across different models. As shown in the our table 2, the probability of Gemini and Llama be regarded as Human written under trained GPT and Human model are 47.4% and 61.3% respectively, indicating nearly half of the texts generated by Gemini and Llama are wrongly classified as human written, which shows a weak tendency of classifying texts. In contrast, the classifiers trained using Human and Gemini, as well as the one trained on Human and Llama, have obvious tendencies to classify the other two LLM-generated models. In particular, the proportion of correctly classified as LLM-generated texts exceeded 90% basing on both tested data from GPT and Llama.

```{r Figure 5, echo=FALSE, warning=FALSE, fig.width = 8, fig.height = 4}
