...
```{r Table 1, fig.cap="Basic Information of The Training Data"}
T1
```
...
```{r Figure 1, fig.cap="Top 10 Mean Difference in Function Words Between LLM and Human Texts"}
...
```{r Figure 2, fig.cap="MDS Visualisation of Human and LLM Texts"}
...
```{r Figure 3, fig.cap="MDS Visualisation of Authors"}
plot(pts, 
     type="n", 
...
\centering
```{r, results='asis'}
knitr::kable(knn_loo_table, digits = 3, booktabs = TRUE)
```
\caption{LOOCV Performance of KNN with k=2}
\end{minipage}
\hfill
\begin{minipage}{0.48\linewidth}
\centering
```{r, results='asis'}
knitr::kable(cm_knn_loo$table, booktabs = TRUE)
```
\caption{Loocv confusion matrix for KNN with k=3}
\end{minipage}
\end{table}
...
Based on training data, Table [] and confusion matrix below show that 'DA' performs much better than 'KNN' in LOOCV. The accuracy is 0.874, with both high sensitivity and specificity. So 'DA' gives a fair classification with meaningful accuracy. Also kappa value (0.685) indicates substantial agreement.
...
```
\caption{LOOCV Performance of DA}
\end{minipage}
\hfill
\begin{minipage}{0.48\linewidth}
\centering
...
Now we start to classify testing data. Since testset is independent with trainset and separated randomly, we have evidence to say that for any given texts, the model is able to classify them between AI and Human at accuracy 95% confidence interval 0.841-0.888, with out obvious bias on AI or Human.
...
```
\caption{Testset classification performance from DA}
\end{minipage}
\hfill
\begin{minipage}{0.48\linewidth}
\centering
...
knitr::kable(cm_da_test$table,
             caption = "DA (Test)")
```
\caption{Testset classification confusion matrix from DA}
\end{minipage}
\end{table}
